...
class SimpleCache:
    """Minimal append-only KV cache (cat-based) for correctness checking."""
    def __init__(self):
        self.k = self.v = None
...
def main():
    ...
    caches = [SimpleCache() for _ in range(mine.cfg.num_layers)]
    ...
    logits = mine(emb, pos.to(dev), causal, caches)[:, -1, :]
    ...
    for _ in range(N - 1):
        ...
        logits = mine(e, p, None, caches)[:, -1, :]  # full attention over cache
